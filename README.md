...
- `crud/<feature>.py` 风格的数据库 CRUD 与查询封装
...
`services/<feature>.py`。数据库增删改查和查询封装放在
`crud/<feature>.py`，供 service 层调用，不负责 HTTP schema、业务流程编排
或数据库连接管理。路由层可以调用 service 层；service 层不要反向依赖
